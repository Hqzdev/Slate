...
export async function POST(request: NextRequest, context: { params: Promise<{ workspaceId: string }> }) {
  ...
  if (!user.emailVerifiedAt) return NextResponse.json({ error: "Verify your email before transferring ownership" }, { status: 403 });
  ...
  if (!recipient?.emailVerifiedAt) return NextResponse.json({ error: "The new owner must verify their email first" }, { status: 403 });
