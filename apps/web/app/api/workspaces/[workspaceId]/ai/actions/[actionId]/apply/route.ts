...
export async function POST(request: NextRequest, context: { params: Promise<{ actionId: string; workspaceId: string }> }) {
  const denied = await guardMutationRequest(request, { limit: 30, scope: "ai:actions:apply", windowMs: 60_000 });
  ...
  try {
    ...
    const result = await aiAssistantService.applyAction(user.id, workspaceId, actionId);
