...
export async function POST(request: NextRequest, context: { params: Promise<{ taskId: string; workspaceId: string }> }) {
  const denied = await guardMutationRequest(request, { limit: 30, scope: "ai:agent:next", windowMs: 60_000 });
  ...
  try {
    ...
    return NextResponse.json({ agentTask: await aiAgentTaskService.executeNext(user.id, workspaceId, taskId) });
  } catch (error) {
