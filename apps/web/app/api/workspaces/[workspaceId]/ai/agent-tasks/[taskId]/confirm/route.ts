...
export async function POST(request: NextRequest, context: { params: Promise<{ taskId: string; workspaceId: string }> }) {
  const denied = await guardMutationRequest(request, { limit: 6, scope: "ai:agent:confirm", windowMs: 60_000 });
  ...
  try {
    ...
    return NextResponse.json({ agentTask: await aiAgentTaskService.confirm(user.id, workspaceId, taskId) });
  } catch (error) {
