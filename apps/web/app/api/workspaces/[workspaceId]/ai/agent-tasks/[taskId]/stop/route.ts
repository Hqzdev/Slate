...
export async function POST(request: NextRequest, context: { params: Promise<{ taskId: string; workspaceId: string }> }) {
  const denied = await guardMutationRequest(request, { limit: 12, scope: "ai:agent:stop", windowMs: 60_000 });
  ...
  try {
    ...
    return NextResponse.json({ agentTask: await aiAgentTaskService.stop(user.id, workspaceId, taskId) });
  } catch (error) {
